#[cfg(not(feature = "sybase_odbc_spike"))]
fn main() {
    eprintln!(
        "Enable the sybase_odbc_spike feature to run this example:\n\
         SYBASE_ODBC_CONN='Driver={{...}};Server=...;UID=...;PWD=...;' \\\n\
         SYBASE_QUERY='select * from my_table' \\\n\
         cargo run -p connectorx --features sybase_odbc_spike --example sybase_odbc_spike"
    );
    std::process::exit(2);
}

#[cfg(feature = "sybase_odbc_spike")]
...
mod spike {
    ...
    pub fn run() -> Result<()> {
        let conn_str = env::var("SYBASE_ODBC_CONN").context(
            "SYBASE_ODBC_CONN is required, for example \
             Driver={FreeTDS};Server=localhost;Port=5000;Database=mydb;UID=sa;PWD=secret;",
        )?;
        let query = env::var("SYBASE_QUERY").unwrap_or_else(|_| "select 1".to_string());
        let batch_size = parse_env_usize("SYBASE_BATCH_SIZE", 8192)?;
        let max_str_len = parse_env_usize("SYBASE_MAX_STR_LEN", 4096)?;
        let sample_limit = parse_env_usize("SYBASE_SAMPLE_ROWS", 5)?;

        println!("sybase odbc spike");
        ...
        let connection = environment
            ...
            .context("connect with SYBASE_ODBC_CONN")?;

        let Some(mut cursor) = connection
            .execute(&query, (), None)
            .context("execute SYBASE_QUERY")?
        else {
