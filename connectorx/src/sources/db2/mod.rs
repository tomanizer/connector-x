...
pub fn db2_conn_string(conn: &str) -> String {
    let trimmed = conn.trim_start();
    let lower = trimmed.to_ascii_lowercase();
    if lower.starts_with("driver=") || lower.starts_with("dsn=") || lower.starts_with("database=") {
...
fn format_odbc_driver(driver: &str) -> String {
    if driver.starts_with('{') || driver.contains('/') || driver.contains('\\') {
        driver.to_string()
    } else {
        format!("{{{}}}", driver)
    }
